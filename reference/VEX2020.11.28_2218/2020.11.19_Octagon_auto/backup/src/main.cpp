...
int main() {
  ...
  while(true){
    ...
    if(global_counter%20==0)getGyro();
    //getballColor();
...
void autoChassisAdjust(char mode){
      if (GyroDifSum > 0.3 || GyroDifSum < -0.3){  //update left and right offset to make it run directly
        if (mode == 'f'){
...
void setChassisVolt(){   //设置底盘电压
  ...
  double limVolt = max(max(lfVolt,lbVolt), max(rfVolt,rbVolt));     //get maximun speed
  limVolt /=12000;
